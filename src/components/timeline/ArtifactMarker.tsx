...
export function ArtifactMarker({ artifact }: ArtifactMarkerProps) {
  return (
    <article className="grid gap-3 border-t border-carbon/10 pt-4">
      ...
      <div className="grid gap-2 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] md:items-start">
        ...
        <div className="grid gap-2">
          <p className="m-0 text-sm leading-relaxed text-carbon/74">{artifact.summary}</p>
          {artifact.status ? (
            <p className="m-0 font-mono text-[0.62rem] uppercase tracking-[0.18em] text-cobalt">
              {artifact.status}
            </p>
          ) : null}
        </div>
